  ...
  <div>
    ...
    <div style="padding: 4px 0">
      ...
      <span style="float: right">
            <el-input style="width: 200px;"placeholder="请输入内容" v-model="typename" ></el-input> <el-button style="margin-left: 5px" type="primary" icon="el-icon-search" @click="load" >搜索</el-button>
            </span>
    </div>
    <!--          表单-->
    <el-table :data="tableData" header-cell-class-name="headerBg" @selection-change="handleSelectionChange" >
      ...
      <el-table-column prop="typemaxnum" label="类型数量"></el-table-column>
    ...
    <el-dialog title="添加类型" :visible.sync="dialogFormVisible" width="30%">
      <el-form :model="form" label-width="78px">
        ...
        <el-form-item label="类型数量" >
          <el-input v-model="form.typemaxnum" autocomplete="off"></el-input>
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
    load(){
      request.get("http://localhost:9090/bookstype/page",{
        params: {
    ...
    save(){
      request.post("http://localhost:9090/bookstype",this.form).then(res=>{
    ...
    handleDelet(id){
      request.delete("http://localhost:9090/bookstype/"+id).then(res=>{
    ...
    delbatch(){
      ...
      console.log(ids)
      request.post("http://localhost:9090/bookstype/del/batch",ids).then(res =>{
        if(res){this.$message.success("删除成功")
          this.load();
        }else{this.$message.error("删除失败")}
      })
